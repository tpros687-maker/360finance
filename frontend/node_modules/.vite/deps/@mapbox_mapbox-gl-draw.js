...
function sortFeatures(features) {
  return features.map((feature2) => {
    if (feature2.geometry.type === geojsonTypes.POLYGON) {
      feature2.area = import_geojson_area.default.geometry({
...
function featuresAt(event, bbox, ctx, buffer) {
  ...
  const features = ctx.map.queryRenderedFeatures(box, queryParams).filter((feature2) => META_TYPES.indexOf(feature2.properties.meta) !== -1);
  const featureIds = new string_set_default();
  const uniqueFeatures = [];
  features.forEach((feature2) => {
    const featureId = feature2.properties.id;
...
ModeInterface.prototype.addFeature = function(feature2, opts = {}) {
  return this._ctx.store.add(feature2, opts);
};
...
ModeInterface.prototype.isInstanceOf = function(type, feature2) {
  if (type === geojsonTypes.POINT) return feature2 instanceof point_default;
  if (type === geojsonTypes.LINE_STRING) return feature2 instanceof line_string_default;
  if (type === geojsonTypes.POLYGON) return feature2 instanceof polygon_default;
  if (type === "MultiFeature") return feature2 instanceof multi_feature_default;
...
function events_default(ctx) {
  ...
  const isKeyModeValid = (event) => {
    const isBackspaceKey2 = isBackspaceKey(event);
    const isDeleteKey2 = isDeleteKey(event);
    const isDigitKey2 = isDigitKey(event);
    return !(isBackspaceKey2 || isDeleteKey2 || isDigitKey2);
  };
...
function render() {
  ...
  function renderFeature(id, source) {
    const feature2 = store.get(id);
    const featureInternal = feature2.internal(mode);
      ...
      renderRequest = requestAnimationFrame(() => {
        ...
        if (this._emitSelectionChange) {
          this.ctx.events.fire(events.SELECTION_CHANGE, {
            features: this.getSelected().map((feature2) => feature2.toGeoJSON()),
...
Store.prototype.featureCreated = function(featureId, options = {}) {
  ...
  if (silent !== true) {
    const feature2 = this.get(featureId);
    this.ctx.events.fire(events.CREATE, {
      features: [feature2.toGeoJSON()]
    });
  }
  return this;
};
...
Store.prototype.add = function(feature2, options = {}) {
  this._features[feature2.id] = feature2;
  this._featureIds.add(feature2.id);
  this.featureCreated(feature2.id, { silent: options.silent });
  return this;
};
...
Store.prototype.getSelectedCoordinates = function() {
  const selected = this._selectedCoordinates.map((coordinate) => {
    const feature2 = this.get(coordinate.feature_id);
    return {
...
function refreshSelectedCoordinates(store, options = {}) {
  const newSelectedCoordinates = store._selectedCoordinates.filter((point2) => store._selectedFeatureIds.has(point2.feature_id));
...
function coordEach(geojson, callback, excludeWrapCoord) {
  ...
  var j, k, l, geometry, stopG, coords, geometryMaybeCollection, wrapShrink = 0, coordIndex = 0, isGeometryCollection, type = geojson.type, isFeatureCollection = type === "FeatureCollection", isFeature2 = type === "Feature", stop = isFeatureCollection ? geojson.features.length : 1;
...
// node_modules/@turf/projection/dist/esm/index.js
...
function createSupplementaryPoints(geojson, options = {}, basePath = null) {
  ...
  function processLine(line, lineBasePath) {
    ...
    line.forEach((point2, pointIndex) => {
      ...
      const vertex = create_vertex_default(featureId, point2, pointPath, isSelectedPath(pointPath));
...
function extent(feature2) {
  const depth = {
    ...
    MultiLineString: 2,
    MultiPolygon: 3
  }[feature2.geometry.type];
  const coords = [feature2.geometry.coordinates].flat(depth);
...
function move_features_default(features, delta) {
  const constrainedDelta = constrain_feature_movement_default(features.map((feature2) => feature2.toGeoJSON()), delta);
  features.forEach((feature2) => {
...
SimpleSelect.fireActionable = function() {
  ...
  const multiFeatures = selectedFeatures.filter(
    (feature2) => this.isInstanceOf("MultiFeature", feature2)
  );
...
SimpleSelect.onCombineFeatures = function() {
  ...
  for (let i = 0; i < selectedFeatures.length; i++) {
    const feature2 = selectedFeatures[i];
    if (feature2.type.replace("Multi", "") !== featureType) {
      return;
    }
    if (feature2.type.includes("Multi")) {
      feature2.getCoordinates().forEach((subcoords) => {
...
SimpleSelect.onUncombineFeatures = function() {
  ...
  for (let i = 0; i < selectedFeatures.length; i++) {
    const feature2 = selectedFeatures[i];
    if (this.isInstanceOf("MultiFeature", feature2)) {
      feature2.getFeatures().forEach((subFeature) => {
        this.addFeature(subFeature);
        subFeature.properties = feature2.properties;
        createdFeatures.push(subFeature.toGeoJSON());
        this.select([subFeature.id]);
      });
      this.deleteFeature(feature2.id, { silent: true });
...
DirectSelect.onSetup = function(opts) {
  ...
  const feature2 = this.getFeature(featureId);
  if (!feature2) {
...
function api_default(ctx, api) {
  ...
  api.getFeatureIdsAt = function(point2) {
    const features = features_at_default.click({ point: point2 }, null, ctx);
    return features.map((feature2) => feature2.properties.id);
  };
  ...
  api.getSelected = function() {
    return {
      type: geojsonTypes.FEATURE_COLLECTION,
      features: ctx.store.getSelectedIds().map((id) => ctx.store.get(id)).map((feature2) => feature2.toGeoJSON())
    };
  };
  ...
  api.add = function(geojson) {
    ...
    const ids = featureCollection.features.map((feature2) => {
      feature2.id = feature2.id || generateID();
      if (feature2.geometry === null) {
        throw new Error("Invalid geometry: null");
      }
      if (ctx.store.get(feature2.id) === void 0 || ctx.store.get(feature2.id).type !== feature2.geometry.type) {
        const Model = featureTypes[feature2.geometry.type];
        if (Model === void 0) {
          throw new Error(`Invalid geometry type: ${feature2.geometry.type}.`);
        }
        const internalFeature = new Model(ctx, feature2);
        ctx.store.add(internalFeature, { silent });
      } else {
        const internalFeature = ctx.store.get(feature2.id);
        const originalProperties = internalFeature.properties;
        internalFeature.properties = feature2.properties;
        if (!(0, import_fast_deep_equal.default)(originalProperties, feature2.properties)) {
          ctx.store.featureChanged(internalFeature.id, { silent });
        }
        if (!(0, import_fast_deep_equal.default)(internalFeature.getCoordinates(), feature2.geometry.coordinates)) {
          internalFeature.incomingCoords(feature2.geometry.coordinates);
        }
      }
      return feature2.id;
    });
  ...
  api.get = function(id) {
    const feature2 = ctx.store.get(id);
    if (feature2) {
  ...
  api.getAll = function() {
    return {
      type: geojsonTypes.FEATURE_COLLECTION,
      features: ctx.store.getAll().map((feature2) => feature2.toGeoJSON())
    };
  };
