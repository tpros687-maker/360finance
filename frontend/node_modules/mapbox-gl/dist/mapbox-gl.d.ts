...
type PropertyExpressionSpecification = {
...
declare class StructArrayLayout2i2ui8 extends StructArray implements IStructArrayLayout {
	uint8: Uint8Array;
	int16: Int16Array;
	uint16: Uint16Array;
	_refreshViews(): void;
...
declare class StructArrayLayout1i2 extends StructArray implements IStructArrayLayout {
	uint8: Uint8Array;
	int16: Int16Array;
	_refreshViews(): void;
	emplaceBack(v0: number): number;
...
type AppearancePaintProps = {
	...
	"icon-translate": DataDrivenProperty<[
		number,
		number
	]>;
	...
	"text-translate": DataDrivenProperty<[
		number,
		number
	]>;
...
type AppearanceProps = AppearanceLayoutProps;
...
declare class SymbolAppearance {
	...
	private readonly _conditionSpec;
	private readonly _propertiesSpec;
	...
	getName(): string;
	getLayoutProperty(name: keyof AppearanceLayoutProps): PossiblyEvaluatedPropertyValue<ResolvedImage> | PossiblyEvaluatedPropertyValue<number> | PossiblyEvaluatedPropertyValue<[
		number,
		number
	]>;
	getPaintProperty(name: keyof AppearancePaintProps): PossiblyEvaluatedPropertyValue<Color> | PossiblyEvaluatedPropertyValue<number> | PossiblyEvaluatedPropertyValue<string> | PossiblyEvaluatedPropertyValue<[
		number,
		number
	]>;
	getUnevaluatedLayoutProperties(): Layout<AppearanceLayoutProps>;
	getUnevaluatedLayoutProperty(name: keyof AppearanceLayoutProps): PropertyValue$1<ResolvedImage, PossiblyEvaluatedPropertyValue<ResolvedImage>> | PropertyValue$1<number, PossiblyEvaluatedPropertyValue<number>> | PropertyValue$1<[
		number,
		number
	...
	getUnevaluatedPaintProperty(name: keyof AppearancePaintProps): PropertyValue$1<number, PossiblyEvaluatedPropertyValue<number>> | PropertyValue$1<[
		number,
		number
	], PossiblyEvaluatedPropertyValue<[
		number,
		number
	]>> | PropertyValue$1<Color, PossiblyEvaluatedPropertyValue<Color>> | PropertyValue$1<string, PossiblyEvaluatedPropertyValue<string>>;
	...
	hasIconLayoutProperties(): boolean;
	hasTextLayoutProperties(): boolean;
	hasIconPaintProperties(): boolean;
	hasTextPaintProperties(): boolean;
	hasLayoutProperty(name: keyof AppearanceLayoutProps): boolean;
	hasPaintProperty(name: keyof AppearancePaintProps): boolean;
}
/**
...
declare const WebGL2BlendFuncExtended: {
	readonly SRC1_COLOR_WEBGL: 35065;
	readonly SRC1_ALPHA_WEBGL: 34185;
	readonly ONE_MINUS_SRC1_COLOR_WEBGL: 35066;
	readonly ONE_MINUS_SRC1_ALPHA_WEBGL: 35067;
	readonly MAX_DUAL_SOURCE_DRAW_BUFFERS_WEBGL: 35068;
};
type WebGL2BlendFuncExtended$1 = typeof WebGL2BlendFuncExtended;
type WebGL2BlendFuncExtendedConstants = WebGL2BlendFuncExtended$1["SRC1_COLOR_WEBGL"] | WebGL2BlendFuncExtended$1["SRC1_ALPHA_WEBGL"] | WebGL2BlendFuncExtended$1["ONE_MINUS_SRC1_COLOR_WEBGL"] | WebGL2BlendFuncExtended$1["ONE_MINUS_SRC1_ALPHA_WEBGL"];
...
declare class ColorAttachment extends FramebufferAttachment<WebGLTexture> {
	...
	constructor(context: Context, parent: WebGLFramebuffer, attachmentIndex?: number);
	setDirty(): void;
	set(v?: WebGLTexture | null): void;
}
...
declare class Framebuffer {
	...
	static createWithTexture(context: Context, existingFbo: Framebuffer | null, width: number, height: number, attachStencil?: boolean): Framebuffer;
	createColorAttachment(context: Context, idx: number): void;
	removeColorAttachment(context: Context, idx: number): void;
	destroy(): void;
}
...
declare class Context {
	...
	maxPointSize: number;
	extBlendFuncExtended: WebGL2BlendFuncExtended$1 | null;
	forceManualRenderingForInstanceIDShaders: boolean;
	...
	createFramebuffer(width: number, height: number, numColorAttachments: number, depthType?: DepthBufferType | null): Framebuffer;
...
type UniformBindings = Record<PropertyKey, IUniform<any>>;
...
type ImageVersionsMap = Map<string, number>;
declare class AtlasContentDescriptor {
	hash: number;
	requiresMipMaps: boolean;
	private iconDescriptors;
	private patternDescriptors;
	constructor(icons: StyleImageMap<StringifiedImageVariant>, patterns: StyleImageMap<StringifiedImageVariant>, imageVersions: ImageVersionsMap, lut: LUT$1 | null, variantCache?: Map<StringifiedImageVariant, ImageVariant>);
	/**
...
declare class ImageAtlasReference {
...
declare class ImageAtlas {
	...
	constructor(icons: StyleImageMap<StringifiedImageVariant>, patterns: StyleImageMap<StringifiedImageVariant>, lut: LUT$1 | null, imageVersions?: ImageVersionsMap);
	addImages(images: StyleImageMap<StringifiedImageVariant>, positions: ImagePositionMap, padding: number, bins: Array<Rect>): void;
	patchUpdatedImages(imageManager: ImageManager, texture: Texture, scope: string, lut?: LUT$1 | null): void;
...
declare class ImageAtlasCache {
	private cache;
	private finalizationRegistry;
	private textures;
	private textureAccessTimes;
	private textureMemoryUsed;
	private maxTextureMemory;
	constructor(options?: {
...
type LayerConfigs = Record<string, LayerSpecification>;
...
export interface TileProvider<T> {
	...
	loadTile: (tile: {
		...
		request: RequestParameters;
		signal: AbortSignal;
	}) => Promise<TileDataResponse<T | null> | null | undefined>;
}
declare function addTileProvider(name: string, url: string): void;
...
type ActorMessages = {
	...
	"getImages": {
		...
		callback: ActorCallback<{
			images: StyleImageMap<StringifiedImageId>;
...
declare class MapWorker {
	...
	setContextParams(mapId: number, params: ActorMessages["setContextParams"]["params"], callback: ActorMessages["setContextParams"]["callback"]): void;
	 ...
	 * @private
	 */
	loadTileProvider(mapId: number, params: ActorMessages["loadTileProvider"]["params"], callback: ActorMessages["loadTileProvider"]["callback"]): void;
	/**
	...
	setConfig(mapId: number, updates: ActorMessages["setConfig"]["params"]): void;
...
declare class ImageManager extends Evented {
	...
	getImages(ids: ImageId[], scope: string, callback: Callback<{
		images: StyleImageMap<StringifiedImageId>;
		versions: Map<string, number>;
	}>): void;
	...
	_notify(ids: ImageId[], scope: string, callback: Callback<{
		images: StyleImageMap<StringifiedImageId>;
		versions: Map<string, number>;
	}>): void;
...
declare class Terrain extends Evented {
	...
	set(terrain: TerrainSpecification | TerrainSpecificationUpdate, configOptions?: ConfigOptions | null): void;
	updateTransitions(parameters: TransitionParameters): void;
	hasTransition(): boolean;
	recalculate(parameters: EvaluationParameters): void;
...
declare class SymbolPropertiesUBO {
	static readonly HEADER_DWORDS = 12;
	static readonly HEADER_BYTES = 48;
	propsDwords: number;
	totalBytes: number;
	headerData: Uint32Array;
	propertiesData: Float32Array;
	blockIndicesData: Uint32Array;
	headerBuffer: WebGLBuffer | null;
	propertiesBuffer: WebGLBuffer | null;
	blockIndicesBuffer: WebGLBuffer | null;
	batchIndex: number;
	context: Context | null;
	constructor(context?: Context | null | undefined, batchIndex?: number, uboSizeDwords?: number);
	 ...
	 */
	static getMaxFeatureCount(header: SymbolPropertyHeader, propsDwords?: number): number;
	/**
...
declare class SymbolStyleLayer extends StyleLayer {
	...
	_handleOverridablePaintPropertyUpdate<T, R>(name: string, oldValue: PropertyValue$1<T, R>, newValue: PropertyValue$1<T, R>): boolean;
...
type LocalFeatureVertexRange = {
	batchIndex: number;
	localFeatureIndex: number;
	vtFeatureIndex: number;
	featureId: string | number | undefined;
};
type ConstantUniformValues = {
	fill_np_color: [
		number,
		number,
		number,
...
declare class SymbolPropertyBinderUBO {
	...
	constructor(layer: SymbolStyleLayer, zoom: number, lut: LUT$1 | null, isText: boolean, worldview?: string, maxUniformBufferBindings?: number | null, uboSizeDwords?: number | null);
	/**
	 ...
	 *   zoom-dep → [min, max]
	 */
	evaluateAllProperties(feature: Feature, featureState: FeatureState, canonical: CanonicalTileID, availableImages: ImageId[], brightness?: number | null, formattedSection?: FormattedSection, activeAppearance?: SymbolAppearance | null): Array<PropertyValue | null>;
	/**
	 ...
	 */
	private _appearancesHavePaintProperties;
	private _checkIfAllPropertiesAreConstant;
	/**
	 ...
	 */
	populateUBO(feature: Feature, vtFeatureIndex: number, canonical: CanonicalTileID, availableImages: ImageId[], brightness?: number | null, formattedSection?: FormattedSection, context?: Context): number;
	/**
	 * Update specific features when feature-state changes.
	 */
	updateFeatures(featureIds: Set<string | number>, styleLayer: SymbolStyleLayer, vtLayer: VectorTileLayer, canonical: CanonicalTileID, availableImages: ImageId[], featureStates: {
		[key: string | number]: FeatureState;
	}, brightness?: number | null): void;
	/**
	 ...
	 */
	updateDynamicExpressions(styleLayer: SymbolStyleLayer, vtLayer: VectorTileLayer, canonical: CanonicalTileID, availableImages: ImageId[], featureStates: {
		[key: string | number]: FeatureState;
	}, brightness?: number | null): void;
	/**
	 ...
	 */
	updateFeaturePaintForAppearance(vtFeatureIndex: number, feature: Feature, featureState: FeatureState, canonical: CanonicalTileID, availableImages: ImageId[], brightness: number | null | undefined, activeAppearance: SymbolAppearance | null | undefined): boolean;
	/**
	 ...
	 */
	bind(context: Context, program: WebGLProgram, batchIndex?: number): void;
	/**
...
declare class FeatureAppearances {
	readonly appearancesVersion: number;
	private readonly appearances;
	private readonly anyFeatureDependent;
	private readonly anyStateDependent;
	private lastParams;
	private lastAllFeaturesIndex;
	constructor(appearances: SymbolAppearance[], version: number);
	update(globalProperties: GlobalProperties, featureStateChanged: boolean): AppearanceUpdateResult;
...
declare class SymbolBuffers {
	 ...
	 */
	getBatchGrouping(segments: SegmentVector): {
		batchIndices: number[];
		batchSegments: Map<number, SegmentVector>;
	};
	snapshotSymbolVertexData(offset: number, numVertices: number): Uint16Array;
	restoreSymbolVertexData(offset: number, snapshot: Uint16Array): void;
	updateSymbolVertexData(vertexIndex: number, anchorX: number, anchorY: number, newOx: number, newOy: number, newTx: number, newTy: number, newSizeX: number, newSizeY: number, pixelOffsetX: number, pixelOffsetY: number, minFontScaleX: number, minFontScaleY: number): void;
...
declare class SymbolBucket implements Bucket {
	...
	symbolsByBatch: Map<number, Array<{
		index: number;
		rotatedY: number;
		featureIndex: number;
	}>> | null;
	...
	getAppearanceFeatureData(featureIndex: number): AppearanceFeatureData | undefined;
	createArrays(): void;
	...
	calculateEffectiveAppearanceIconSize(activeAppearance: SymbolAppearance, currentZoom: number, evaluationFeature: EvaluationFeature, canonical: CanonicalTileID, availableImages: Array<ImageId>, iconScaleFactor: number, layoutIconSize: number): number;
	...
	update(states: FeatureStates, vtLayer: VectorTileLayer, availableImages: ImageId[], imagePositions: SpritePositions, layers: ReadonlyArray<TypedStyleLayer>, isBrightnessChanged: boolean, brightness?: number | null, canonical?: CanonicalTileID): void;
	...
	updateAppearances(canonical?: CanonicalTileID, featureState?: FeatureStates, availableImages?: Array<ImageId>, globalProperties?: GlobalProperties, imageManager?: ImageManager, featureStateChanged?: boolean): {
	 ...
	 */
	private getOrCreateSortedSymbolsByBatch;
	/**
	 ...
	 */
	sortFeaturesWithinSegments(): void;
...
declare class PauseablePlacement {
	...
	startNewPlacement(transform: Transform$1, order: Array<string>, showCollisionBoxes: boolean, fadeDuration: number, crossSourceCollisions: boolean, prevPlacement?: Placement, fogState?: FogState | null, buildingIndex?: BuildingIndex | null): PauseablePlacement;
...
declare class ViewportIntersectionStrategy {
	_previousClosestBuildingId: string | null;
	_hysteresisRatio: number;
	_indoorMinimumZoom: number;
	findClosestBuilding(buildings: Record<string, IndoorBuilding>, mapCenter: LngLat, zoom: number, mapBounds: LngLatBounds, viewportPolygon?: Point[]): string | null;
...
declare class IndoorManager extends Evented<IndoorEvents> {
	...
	_updateUI(): void;
	_onBuildingTransition(previousId: string | null, currentId: string | null): void;
	_updateIndoorSelector(): void;
	_updateActiveFloors(visible: boolean): void;
	_recalculateActiveFloors(): void;
	_mergeFloors(target: IndoorFloor, source: IndoorFloor): void;
	_makeViewportPolygon(): Point[];
}
...
export declare class ImageSource<T = "image"> extends Evented<SourceEvents> implements ISource<T> {
	...
	_boundsArray: StructArrayLayout2i2ui8 | null | undefined;
...
type StyleBOM = Array<StyleBOMEntry>;
...
declare class Style$1 extends Evented<MapEvents> {
	...
	_updateDataDrivenEmissiveStrength(): void;
	...
	addSource(id: string, source: (SourceSpecification | CanvasSourceSpecification | CustomSourceInterface<unknown>) & {
		collectResourceTiming?: boolean;
	...
	setWorldview(worldview: string | undefined | null): void;
	...
	setTerrain(terrainOptions?: TerrainSpecification | TerrainSpecificationUpdate | null, drapeRenderMode?: number): void;
	...
	_handleLayerOrderChange(): void;
	_requestFullLabelPlacement(): void;
	_setLabelPlacementStale(): void;
	_updatePlacement(transform: Transform$1, showCollisionBoxes: boolean, fadeDuration: number, crossSourceCollisions: boolean, replacementSource: ReplacementSource): boolean;
	...
	checkAtlasCache(mapId: string, params: ActorMessages["checkAtlasCache"]["params"], callback: ActorMessages["checkAtlasCache"]["callback"]): void;
...
type SymbolDefinesType = "COLOR_ADJUSTMENT" | "ICON_TRANSITION" | "PITCH_WITH_MAP_TERRAIN" | "PROJECTED_POS_ON_VIEWPORT" | "RENDER_SDF" | "RENDER_TEXT_AND_SYMBOL" | "Z_OFFSET" | "APPLY_LUT_ON_GPU" | "USE_PAINT_PROPERTIES_UBO";
...
declare class LineBucket implements Bucket {
	...
	zOffsetVertexBuffer: VertexBuffer;
	elevationGroundScaleVertexArray: StructArrayLayout1f4;
	...
	update(states: FeatureStates, vtLayer: VectorTileLayer, availableImages: ImageId[], imagePositions: SpritePositions, layers: ReadonlyArray<TypedStyleLayer>, isBrightnessChanged: boolean, brightness?: number | null, canonical?: CanonicalTileID, worldview?: string): void;
...
type LineDefinesType = "RENDER_LINE_GRADIENT" | "RENDER_LINE_DASH" | "RENDER_LINE_TRIM_OFFSET" | "RENDER_LINE_BORDER" | "LINE_JOIN_NONE" | "ELEVATED" | "VARIABLE_LINE_WIDTH" | "CROSS_SLOPE_VERTICAL" | "CROSS_SLOPE_HORIZONTAL" | "ELEVATION_REFERENCE_SEA" | "ELEVATION_GROUND_SCALE" | "LINE_PATTERN_TRANSITION" | "USE_MRT1" | "DUAL_SOURCE_BLENDING" | "LINE_BLEND_MULTIPLY";
...
type RasterDefinesType = "RASTER_COLOR" | "RENDER_CUTOFF" | "RASTER_ARRAY" | "RASTER_ARRAY_LINEAR" | "ELEVATION_REFERENCE_GROUND";
type FillExtrusionDefinesType = "CLEAR_FROM_TEXTURE" | "CLEAR_SUBPASS" | "FAUX_AO" | "FLOOD_LIGHT" | "HAS_CENTROID" | "RENDER_FRONT_CUTOFF" | "RENDER_WALL_MODE" | "SDF_SUBPASS" | "ZERO_ROOF_RADIUS" | "FILL_EXTRUSION_PATTERN_TRANSITION" | "HAS_ATTRIBUTE_a_flood_light_ground_radius";
...
type ElevationUniformsType = CircleUniformsType | CollisionUniformsType | DebugUniformsType | FillExtrusionDepthUniformsType | FillExtrusionPatternUniformsType | GlobeRasterUniformsType | GlobeUniformsType | HeatmapUniformsType | LinePatternUniformsType | LineUniformsType | RasterUniformsType | SymbolUniformsType | TerrainRasterUniformsType;
...
declare class Terrain$1 extends Elevation {
	...
	_createFBO(): FBO;
	_updateFBOs(needsEmissiveTexture: boolean): void;
	_updateFBO(fbo: FBO, needsEmissiveTexture: boolean): void;
	_initFBOPool(): void;
...
type BuildingDefinesType = "DEBUG_SHOW_NORMALS" | "HAS_ATTRIBUTE_a_part_color_emissive" | "HAS_ATTRIBUTE_a_bloom_attenuation" | "BUILDING_FAUX_FACADE" | "HAS_ATTRIBUTE_a_faux_facade_color_emissive" | "RENDER_FRONT_CUTOFF";
...
declare class ModelBucket implements Bucket {
	...
	addFeature(feature: BucketFeature, geometry: Array<Array<Point>>, evaluationFeature: EvaluationFeature, elevationFeatures?: ElevationFeature[], canonical?: CanonicalTileID): string;
...
declare class ModelStyleLayer extends StyleLayer {
	...
	queryIntersectsFeature(queryGeometry: TilespaceQueryGeometry, feature: VectorTileFeature, featureState: FeatureState, geometry: Array<Array<Point>>, zoom: number, transform: Transform$1, pixelPosMatrix: Float32Array, elevationHelper: DEMSampler | null | undefined, layoutVertexArrayOffset: number, scope: string | undefined): number | boolean;
	_handleOverridablePaintPropertyUpdate<T, R>(name: string, oldValue: PropertyValue$1<T, R>, newValue: PropertyValue$1<T, R>): boolean;
...
type LineBlendCompositeUniformsType = {
	["u_image"]: Uniform1i;
	["u_opacity"]: Uniform1f;
...
declare const programUniforms: {
	...
	readonly lineBlendComposite: (context: Context) => LineBlendCompositeUniformsType;
...
declare class Program$1<Us extends UniformBindings> {
	...
	fixedUniformsEntries: ReadonlyArray<[
		string,
		IUniform<unknown>
	]>;
...
declare class SegmentVector {
	...
	_prepareSegment(numVertices: number, vertexArrayLength: number, indexArrayLength: number, sortKey?: number, batchIndex?: number): Segment;
	prepareSegment(numVertices: number, layoutVertexArray: StructArray, indexArray: StructArray, sortKey?: number, batchIndex?: number): Segment;
	get(): Array<Segment>;
	destroy(): void;
...
type LayoutProps$4 = {
	...
	"source-max-zoom": DataConstantProperty<number>;
};
type PaintProps$8 = {
	...
	"fill-extrusion-front-cutoff": DataConstantProperty<[
		number,
		number,
		number
	]>;
...
declare class BuildingGeometry {
	...
	layoutFloodLightDataArray: StructArrayLayout1i2;
	...
	reserve(numVertices: number, numIndices: number, withFauxFacade: boolean): void;
}
declare class BuildingBucket implements BucketWithGroundEffect {
	...
	footprintsVertices: StructArrayLayout2f8;
	footprintsIndices: StructArrayLayout1ui2;
	footprintsMin: Point;
	footprintsMax: Point;
...
type PaintProps$9 = {
	...
	"building-front-cutoff": DataConstantProperty<[
		number,
		number,
		number
	]>;
...
type EmissiveMode = "constant" | "dual-source-blending" | "mrt-fallback";
...
declare class Painter {
	...
	constructor(gl: WebGL2RenderingContext, contextCreateOptions: ContextOptions, transform: Transform$1, scaleFactor: number, worldview: string | undefined);
...
declare class SourceCache extends Evented {
	...
	pause(): void;
	resume(): void;
	setMaxzoomOverride(maxzoom: number | null): void;
	_loadTile(tile: Tile, callback: Callback<undefined>): void;
	...
	_tileLoaded(tile: Tile, id: number, previousState: TileState, err?: Error | null, data?: LoadVectorTileResult | null): void;
	/**
...
declare class Transform$1 {
	...
	equals(transform: Transform$1): boolean;
	/**
...
declare class StyleLayer extends Evented {
	...
	queryIntersectsFeature(_queryGeometry: TilespaceQueryGeometry, _feature: VectorTileFeature, _featureState: FeatureState, _geometry: Array<Array<Point>>, _zoom: number, _transform: Transform$1, _pixelPosMatrix: Float32Array, _elevationHelper: DEMSampler | null | undefined, _layoutVertexArrayOffset: number, scope: string | undefined): boolean | number;
}
type PaintProps$10 = {
...
declare class ElevatedStructures {
	...
	addPortalCandidates(id: number, polygons: Point[][][], isTunnel: boolean, elevation: ElevationFeature, zLevel: number): void;
...
interface Bucket {
	...
	destroy: (reload?: boolean) => void;
	updateFootprints: (id: UnwrappedTileID, footprints: Array<TileFootprint>) => void;
	updateAppearances: (canonical?: CanonicalTileID, featureState?: FeatureStates, availableImages?: Array<ImageId>, globalProperties?: GlobalProperties, imageManager?: ImageManager, featureStateChanged?: boolean) => AppearanceUpdateResult$1;
}
interface Vertex {
	position: vec2;
	height: number;
...
interface Property<T, R> {
	...
	possiblyEvaluate: (value: PropertyValue$1<T, R>, parameters: EvaluationParameters, canonical?: CanonicalTileID, availableImages?: ImageId[], iconImageUseTheme?: string) => R;
	interpolate: (a: R, b: R, t: number) => R;
}
declare class PropertyValue$1<T, R> {
...
declare class TransitioningPropertyValue<T, R> {
	...
	constructor(property: Property<T, R>, value: PropertyValue$1<T, R>, prior: TransitioningPropertyValue<T, R> | null | undefined, transition: TransitionSpecification, now: TimePoint);
...
type PropertyValues<Props> = {
	[Key in keyof Props]: Props[Key] extends Property<infer T, infer R> ? PropertyValue$1<T, R> : never;
};
...
declare class PossiblyEvaluatedPropertyValue<T> {
	...
	evaluate(feature: Feature, featureState: FeatureState, canonical?: CanonicalTileID, availableImages?: ImageId[], formattedSection?: FormattedSection): T;
}
...
declare class DataConstantProperty<T> implements Property<T, T> {
	...
	possiblyEvaluate(value: PropertyValue$1<T, T>, parameters: EvaluationParameters): T;
...
declare class DataDrivenProperty<T> implements Property<T, PossiblyEvaluatedPropertyValue<T>> {
	...
	possiblyEvaluate(value: PropertyValue$1<T, PossiblyEvaluatedPropertyValue<T>>, parameters: EvaluationParameters, canonical?: CanonicalTileID, availableImages?: ImageId[], iconImageUseTheme?: string): PossiblyEvaluatedPropertyValue<T>;
	...
	evaluate(value: PossiblyEvaluatedValue<T>, parameters: EvaluationParameters, feature: Feature, featureState: FeatureState, canonical?: CanonicalTileID, availableImages?: ImageId[], iconImageUseTheme?: string, formattedSection?: FormattedSection): T;
}
declare class ColorRampProperty implements Property<Color, boolean> {
	specification: StylePropertySpecification;
	constructor(specification: StylePropertySpecification);
	possiblyEvaluate(value: PropertyValue$1<Color, boolean>, parameters: EvaluationParameters, canonical?: CanonicalTileID, availableImages?: ImageId[]): boolean;
	interpolate(): boolean;
}
declare class DirectionProperty implements Property<[
	...
	specification: StylePropertySpecification;
	constructor(specification: StylePropertySpecification);
	possiblyEvaluate(value: PropertyValue$1<[
		number,
		number
...
type TextureFormat = WebGL2RenderingContext["RGBA8" | "RGBA16F" | "DEPTH_COMPONENT16" | "DEPTH24_STENCIL8" | "R8" | "R32F"];
...
declare class Texture {
	...
	update(image: TextureImage, options?: TextureUpdateOptions | null): void;
...
declare const RenderSourceType: {
	readonly Other: 0;
	readonly Symbol: 1;
	readonly FillExtrusion: 2;
};
type RenderSourceType = typeof RenderSourceType[keyof typeof RenderSourceType];
...
declare class Tile {
	...
	evaluateQueryRenderedFeaturePadding(): number;
	queryRenderedFeatures(query: QrfQuery, tilespaceGeometry: TilespaceQueryGeometry, availableImages: ImageId[], transform: Transform$1, sourceCacheTransform: Transform$1, visualizeQueryGeometry: boolean, scope: string | undefined): QueryResult;
	...
	refreshFeatureState(painter?: Painter, states?: LayerFeatureStates): void;
	hasAppearances(painter: Painter): boolean;
	updateBuckets(painter: Painter, isBrightnessChanged?: boolean, states?: LayerFeatureStates, needsSymbolUBOUpdate?: boolean, updatedPaintProps?: Set<string>, availableImages?: ImageId[]): void;
	holdingForFade(): boolean;
...
export declare class VectorTileSource extends Evented<SourceEvents> implements ISource<"vector"> {
	...
	load(callback?: Callback<undefined>): void;
	_loadWithProvider(tileProvider: {
		name: string;
		url: string;
	}, callback: Callback<TileJSON>): {
		cancel: () => void;
	};
	_setTileJSON(tileJSON: TileJSON): void;
	loaded(): boolean;
	hasTile(tileID: OverscaledTileID): boolean;
	onAdd(map: Map$1): void;
	/**
...
declare class Map$1 extends Camera {
	...
	******************************************************************************/
	...
	_postAddingAppearancesToStyleEvent(): void;
	_updateTerrain(): void;
...
export declare class GeolocateControl extends Evented<GeolocateControlEvents> implements IControl {
	...
	_finish(): void;
	_startRequestTimeout(): void;
	_clearRequestTimeout(): void;
	...
	* });
	*/
	_onDeviceOrientation(deviceOrientationEvent: DeviceOrientationEvent): void;
	/**
...
declare const exported: {
	...
	addTileProvider: typeof addTileProvider;
	Map: typeof Map$1;
